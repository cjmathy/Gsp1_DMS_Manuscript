...
p3 <-
    ...
    inner_join(df_fit, by=c('position','aa_to')) %>% pull(mutant_type)
...
p4 <-
    ...
    filter(sample != 'WT') %>% 
    mutate(mutant_type=factor(type, levels=c('WT-like', 'Toxic/GOF'))) %>% 
    rename('mutant'=sample, 'mutant_type'=mutant_type) %>% 
...
ggsave('figures/FigS5/FigS5C_biophysics.pdf', width = 9, height = 2)
