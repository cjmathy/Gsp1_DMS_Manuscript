...
ggplot(scans_df) +
    geom_point(aes(x=wavelength, y= molar_ellipticity, color=mutant_type), size=0.2, stroke=0.5) +
    scale_color_manual(values=c('red','blue','gray'), name='Mutant Type') +
    xlab('Wavelength') + ylab('Molar ellipticity [deg * cm * dmol-1]') +
    facet_wrap(vars(sample), ncol=4, scales='fixed') +
    ylim(c(-3e5, 3e5)) + 
    theme_custom +
...
ggsave('figures/FigS5/FigS5A_scans.pdf', width = 3, height = 2.8)
