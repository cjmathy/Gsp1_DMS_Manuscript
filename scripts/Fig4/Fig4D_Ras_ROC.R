...
pdf('figures/Fig4/Fig4D_HRas_ROC.pdf', family = 'Helvetica', width=3, height=2.2, pointsize=7)
plot(perf, col='black', lwd=2,
     main='Predictions of H-Ras GOF mutations', 
...
# For Ras activating that are not considered toxic/GOF positions, how
# many toxic mutation are there?

gof_not_tox <- df %>% 
    select('pos'=position_Gsp1) %>%  # only use positions aligned with HRas
    unique() %>% 
    mutate(
        tox = ifelse(pos %in% toxics, T,F),
        sca = ifelse(pos %in% sca_gsp1, T,F),
        gof_all = ifelse(pos %in% gofs_all, T,F),
        gof_2plus = ifelse(pos %in% gofs_2plus, T,F)) %>% 
    filter(gof_2plus, !tox) %>% 
    pull(pos)



df_Gsp1 %>%
    filter(position_Gsp1 %in% gof_not_tox) %>% 
    group_by(position_Gsp1, bin_Gsp1) %>% 
    summarise(n=n()) %>% 
    filter(bin_Gsp1=='toxic')



### print list of positions with one or more activating mutation in Ras
df %>% 
    mutate('activating'=ifelse(score_HRas > activating_threshold, T, F)) %>% 
    filter(activating) %>% 
